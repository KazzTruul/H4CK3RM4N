...
public class XMLManager : MonoBehaviour
{
    ...
    static XPathNavigator currentGameNav, settingsNav;

    static XmlDocument currentGameDoc = new XmlDocument(), settingsDoc;
    ...
    void Awake()
    {
        if (settingsDoc == null)
        {
            ...
            settingsDoc.Load(Application.streamingAssetsPath + "/Settings_XML.xml");
            settingsNav = settingsDoc.CreateNavigator();
        }
    ...
    {
        XPathNavigator volumesNode = settingsNav.SelectSingleNode("/Settings/Audio");
        foreach (XPathNavigator volume in volumesNode.SelectChildren(XPathNodeType.All))
            if (volume.GetAttribute("Name", "") == volumeName)
                return float.Parse(volume.GetAttribute("Value", ""));
        print("No node named " + volumeName + " found.");
        return 0f;
    }

    public static void SaveAudioSetting(string settingName, float value)
    {
        XPathNavigator audioNode = settingsNav.SelectSingleNode("/Settings/Audio");
        foreach (XPathNavigator node in audioNode.SelectChildren(XPathNodeType.All))
        {
            if (node.GetAttribute("Name", "") == settingName)
            {
                node.MoveToAttribute("Value", "");
    ...
    [YarnCommand("NewLoadMessage")]
    public static void NewLoadMessage(string newMessage)
    {
        XPathNavigator loadTextNav = currentGameNav.SelectSingleNode("/Game/LoadText");
        loadTextNav.MoveToAttribute("Text", "");
        loadTextNav.SetValue(newMessage);
    }
    ...
    public static void NewYarnNode(string person, string node)
    {
        ...
        foreach (XPathNavigator availableNode in availableNodes.SelectChildren(XPathNodeType.All))
        {
            if (availableNode.GetAttribute("Name", "") == person)
            {
                ...
                SetNewMessageAvailable(person, node);
                return;
            }
        }
    }
    ...
    public static void SetNewMessageAvailable(string person, string messageNode)
    {
        ...
        XPathNavigator nodes = currentGameNav.SelectSingleNode("/Game/Yarn/AvailableNodes");
